...
interface SubClientSettingsProps {
  projectId: string;
}

export default function SubClientSettingsPage({ projectId }: SubClientSettingsProps) {
  const { user } = useAuth();
  const { projects, currentProject, updateProjectSubClientSettings, error } = useProjectStore();
