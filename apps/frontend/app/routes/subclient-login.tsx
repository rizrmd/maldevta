...
import { subclientLogin } from "~/lib/iam-api";
...
export default function SubclientLoginPage() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setLoading(true);
    try {
      await subclientLogin({ username, password });
      navigate("/");
    } catch (err) {
  ...
  return (
    <main className="mx-auto max-w-md px-4 py-10">
      <h1 className="text-2xl font-semibold">Subclient Login</h1>
      <p className="mt-2 text-sm text-gray-600">
        Use your subclient domain and credentials.
      </p>
